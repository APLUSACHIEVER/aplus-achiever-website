...
const BANK3='APLUS_P6_PSLE_SYNTHESIS_TRANSFORMATION_DNA_V3';
...
function install(){
 const G=window.APLUS_P6_PSLE_PAPER2_GENERATION_V1, B=window[BANK], B2=window[BANK2], B3=window[BANK3];
 if(!G||typeof G.generate!=='function'||!Array.isArray(B)||!Array.isArray(B2)||!Array.isArray(B3)||G.synthesisDNAInstalled)return false;
 const original=G.generate;
 function make(seed){
   const base=shuffle(B.concat(B2,B3),seed);
 ...
 G.generate=function(options={}){
   ...
   r.paper=r.paper.map(q=>{
     ...
     return Object.assign({},q,{
       ...
       sourceDatabase:'APLUS P6 PSLE Synthesis / Transformation DNA V1 + V2 + V3',
       sourceRecordId:x.id,
