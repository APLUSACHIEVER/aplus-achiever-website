...
async function bootstrap(){
  ...
  try{const r=await fetch('prelim-question-generation-layer-v1-20260918.js?v=20260918b');if(!r.ok)throw new Error('Generation layer HTTP '+r.status);exposeSource(await r.text())}catch(e){throw new Error('Could not load the Paper 2 generation engine: '+e.message)}
  ...
  status('Question banks ready · '+results.filter(Boolean).length+'/'+DB_FILES.length+' sources loaded',true);
}
