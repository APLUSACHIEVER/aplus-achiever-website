...
*/
(function(){'use strict';
const GNAME='APLUS_P6_PSLE_PAPER2_GENERATION_V1',BNAME='APLUS_AI_DB_V1_P6_PSLE_VOCABULARY_CLOZE_VISUAL_DNA_BATCH01',VERSION='PSLE_VOCABULARY_CLOZE_DNA_V2.0';
...
function install(){const G=window[GNAME],B=window[BNAME];if(!G||typeof G.generate!=='function'||!Array.isArray(B)||!B.length)return false;if(G.vocabClozeDNA===VERSION)return true;
 const original=G.generate;G.generate=function(options={}){
  ...
  const displayPassage=set.items.reduce((txt,it,i)=>txt.replaceAll('[['+it.word+']','('+String(16+i)+') ______').replaceAll(']]',''),set.passage);
  const qs=set.items.map((it,i)=>{const opts=sh(it.options,r);return {type:'mcq',marks:1,section:'vocabularyCloze',number:16+i,id:'Q'+(16+i),passage:'For each question from 16 to 20, choose the word closest in meaning to the underlined word(s).\n\n'+displayPassage,question:'',options:opts,answer:it.answer,optionStyle:'numeric',vocabularyClozeDNA:VERSION,targetWord:it.word,sourceDatabase:'APLUS Vocabulary Cloze Visual DNA Batch 01',sourceRecordId:set.id,skill:'meaning in context',difficulty:i<2?2:i<4?3:4,explanation:'Choose the option that is closest in meaning to the word represented by the numbered blank in the passage.'}});
  if(qs.some(q=>q.options.length!==4||new Set(q.options.map(norm)).size!==4||!q.options.some(x=>norm(x)===norm(q.answer))))return out;
  const paper=out.paper.filter(q=>q.section!=='vocabularyCloze');paper.splice(15,0,...qs);paper.forEach((q,i)=>{q.number=i+1;q.id='Q'+(i+1)});out.paper=paper;out.diagnostics=out.diagnostics||{};out.diagnostics.vocabularyClozeDNA=VERSION;out.diagnostics.vocabularyClozeFormat='numbered inline blanks + four numbered choices';out.diagnostics.vocabularyClozePassageId=set.id;out.diagnostics.vocabularyClozeTargets=qs.map(q=>q.targetWord);return out;
